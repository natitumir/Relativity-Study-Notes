...
One interesting thing is, given S to be a metric submanifold of M ( I will explain what a metric submanifold is in a bit), we can divide $T_pM$ into two subspaces, one would be a subspace of vectors "tangent" to S, and one "normal" to S. 
...
\begin{definition}
    A (2,3) M-tensor $\tensor{T}{ab}{\quad cde}$ at point p is tangent to S in the second upper index b if for any $\eta \in N^*_p$, $\tensor{T}{ab}{\quad cde} \eta_a =0$. It is normal to S in the third lower index e if for any $X \in \perp_p$ , $\tensor{T}{ab}{\quad cdn}X^n=0$. And so on for tensors of different index structures.
\end{definition}
...
\begin{definition}
    $\tensor{T}{ab}{\quad cde}$ is normal in the first upper index a if $g_{am}\tensor{T}{mb}{\quad cde}$ is normal in the index a. Similarly T is tangent in the second lower index d if $g^{dm}\tensor{T}{ab}{\quad cme}$ is tangent in the index d. And so on for tensors of other index structure. 
\end{definition}
